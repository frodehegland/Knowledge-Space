...
enum ModuleExchange {
    ...
    static func exportSwiftFile(_ archive: ModuleArchive) {
        ...
        panel.message = "Add this file to the Xcode project, then register it by adding the line below to the LibraryViewRegistry.modules array in LibraryViewModule.swift. Using an AI assistant in Xcode? Give it this file and say “please add this view module” — it does both steps."
    ...
    static func exportOrigamiView(_ archive: ModuleArchive) {
        ...
        panel.message = "Another user imports this from Edit Views."
        guard panel.runModal() == .OK, let url = panel.url else { return }
...
#if os(macOS)
/// A target for AppKit controls inside SwiftUI-driven panels: holds a
/// closure and fires it on the control's action.
private final class CopyTrampoline: NSObject {
