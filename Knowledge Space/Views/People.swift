...
// The contact system carried over from Digital Letters: the same Person
// records, the same People.json in the shared folder, so both apps —
// and the phone — read and write one directory.
...
nonisolated struct Person: Codable, Identifiable, Hashable, Sendable {
    ...
    var aliases: [String]?
    /// The person's image in the community folder, as a path relative to
    /// it ("Portraits/<localID>.png") — carried in People.json so every
...
/// phone, the headset, and Digital Letters find the community's contact
/// information.
