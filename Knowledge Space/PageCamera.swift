...
struct PageCaptureView: View {
    ...
    private func attempt() async {
        ...
        // A found page takes a window of its own — the main list stays
        // exactly as the reader left it. The reader consumes the
        // pending fragment on arrival and scrolls to the paragraph;
        // Scroll mode so the landing is a flow to the very line.
        // (Paragraph ids are only trustworthy in the revision they
        // were matched against — as in AppState.open.)
        UserDefaults.standard.set(OrigamiReadingView.ReaderMode.scroll.rawValue,
                                  forKey: "readerMode")
        let target = state.index.latestRevision(of: hit.docID)
        state.pendingFragment = target == hit.docID ? hit.paragraphID : nil
        openWindow(id: "reading", value: target)
        ...
        // reading.
        var captured = 0
        if let entry = state.index.allByID[target] {
            ...
            captured = state.captureUnderlines(phrases, in: entry.doc)
        }
        let kept = captured == 0 ? "" :
            " — \(captured) underlined \(captured == 1 ? "passage" : "passages") kept"
        state.showNote("Opened \u{201C}\(hit.title)\u{201D} at the page\u{2019}s place\(kept).")
        status = "Opened \u{201C}\(hit.title)\u{201D}\(kept)."
        // The grey moment lingers long enough to be read, then the
        // living picture returns for the next page.
        await resumeScanning(after: .seconds(3))
    }
    ...
    private func libraryCandidates() async -> [PageMatch.Candidate] {
        ...
        let docs = state.index.allByID.values.map { entry in
            (id: entry.doc.id,
