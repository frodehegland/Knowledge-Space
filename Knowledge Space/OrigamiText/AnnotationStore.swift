...
// Ported verbatim from Augmented Library's AnnotationStore.swift — keep
// synced; a fix here should be carried back. In Knowledge Space the
// sidecar lives beside the document's own file in the community folder.

/// Web annotations live beside the source records in the Augmented
/// Library store: one JSON-LD sidecar per annotated document,
/// `<address>.annotations.jsonld`, holding a W3C AnnotationCollection.
/// They are never written into the document or its published EPUB — the
/// document is the author's; the annotations are the reader's.
public nonisolated enum AnnotationStore {
    ...
    /// Every annotation in the document's sidecar, oldest first. A missing
    ...
    /// inline (no paging — a reader's notes on one document stay small).
