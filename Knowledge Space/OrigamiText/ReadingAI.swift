...
// Ported from Augmented Library's ReadingAI.swift — keep synced; a fix
// here should be carried back. One Knowledge Space adaptation, marked
// below: availability asks the system model directly (Augmented
// Library routes the same question through its SourceAnalysis).
...
/// `@AppStorage("readingAIPrompts")` and edited in Settings → AI.
...
public nonisolated enum ReadingAI {
    ...
    /// Whether the on-device model can run here. (Knowledge Space:
    /// the system model answers directly.)
