...
// Ported from Augmented Library's OrigamiEPUBImport.swift (itself
// ported verbatim from Origami Text's) so an Origami EPUB opens
// identically in all three apps — access levels dropped, plus one
// addition worth carrying back: a plain EPUB with several spine
// documents (a chaptered book) now reads whole, every spine document
// in order, instead of its first file alone. A fix here should be
// carried to the other two homes.
...
nonisolated enum OrigamiEPUBImporter {
    ...
    struct ImportResult: Sendable {
        ...
        var excerptOf: LiquidDoc.ExcerptOf? = nil
    ...
    static func importDocument(at url: URL) throws -> ImportResult {
        ...
        // The metadata payload: Visual-Meta (the older export) or
        // origami.json (the current Author export) — the package file
        // first, the embedded copy second.
        let visualMetaData = zip.entry("visual-meta.json")
            ?? zip.entries.first { $0.key.hasSuffix("visual-meta.json") }?.value
            ?? zip.entry(joinedPath(opfDirectory, "origami.json"))
        ...
        // The current Author export ships its glossary as a dictionary
        // keyed by id — a phrase and its explanation each. Those are
        // concepts in the document model's terms.
        if let glossary = visualMeta?["glossary"] as? [String: Any] {
            let fromGlossary = glossary.compactMap { id, value -> LiquidDoc.Concept? in
                guard let node = value as? [String: Any],
                      let phrase = node["phrase"] as? String, !phrase.isEmpty else { return nil }
                return LiquidDoc.Concept(
                    id: id,
                    name: phrase,
                    description: node["entry"] as? String ?? "",
                    tag: (node["tags"] as? [String])?.first.flatMap { $0.isEmpty ? nil : $0 },
                    citationIdentifiers: node["citations"] as? [String] ?? [],
                    urls: (node["urls"] as? [String] ?? []).filter { !$0.isEmpty })
            }
            .sorted { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending }
            concepts.append(contentsOf: fromGlossary)
        }
        ...
        let contentPath = joinedPath(opfDirectory, contentHref)
        let contentDir = (contentPath as NSString).deletingLastPathComponent
        ...
        if visualMeta == nil, spineHrefs.count > 1 {
            ...
            for (index, href) in spineHrefs.enumerated() {
                guard let data = zip.entry(joinedPath(opfDirectory, href))
                        ?? zip.entry(href) else { continue }
                ...
                let resolve: (String) -> Data? = { src in
                    ...
                    guard let bytes = zip.entry(full)
                            ?? zip.entry(src)
                            ?? zip.entries.first(where: {
                                $0.key.hasSuffix("/\((src as NSString).lastPathComponent)")
                            })?.value,
        ...
        // Authors: the older export writes ["Name"], the current one
        // [{"name": "Name"}].
        let documentAuthor: String? = {
            if let names = document?["authors"] as? [String] { return names.first }
            if let people = document?["authors"] as? [[String: Any]] {
                return people.first?["name"] as? String
            }
            return nil
        }()
        // The excerpt declaration, when this EPUB is a section carved
        // out of another document.
        var excerptOf: LiquidDoc.ExcerptOf?
        if let excerpt = visualMeta?["excerptOf"] as? [String: Any],
           let originalID = excerpt["id"] as? String, !originalID.isEmpty,
           let headingID = excerpt["headingID"] as? String, !headingID.isEmpty {
            excerptOf = LiquidDoc.ExcerptOf(
                id: originalID,
                title: excerpt["title"] as? String ?? "",
                author: excerpt["author"] as? String ?? "",
                date: excerpt["date"] as? String,
                headingID: headingID,
    ...
    /// chapters of a plain, multi-document EPUB.
    ...
    static func embeddedVisualMeta(in html: String) -> Data? {
        guard let open = html.range(of: "id=\"visual-meta-payload\">")
                ?? html.range(of: "id=\"origami-metadata\">"),
    ...
    /// The media type an image file extension implies (inlined from
    /// Origami Text's WordImporter helper).
    ...
    private static func bodyParagraphs(fromXHTML html: String,
        ...
        func visit(_ element: XMLTree.Element, stretchID: String? = nil) {
            // <h1> is the current export's document-title heading; with
            // that echo skipped below, <h2> is the top rank in both
            // profiles.
            let headingLevels = ["h1": 1, "h2": 1, "h3": 2, "h4": 3]
            let stableID: () -> String = {
                fallbackOrdinal += 1
            ...
            case "h1", "h2", "h3", "h4":
                ...
                // <h1>; the title already heads the imported card, so the
                // leading echo stays out of the body.
                if element.name == "h1", paragraphs.isEmpty, let documentTitle,
                   text.caseInsensitiveCompare(documentTitle) == .orderedSame {
                    return
                }
                // The section's own writer, when Author credited one
                // (cmd-click on the heading).
                let contributing = element.attributes["data-contributing-authors"]?
                    .trimmingCharacters(in: .whitespacesAndNewlines)
                paragraphs.append(LiquidDoc.Paragraph(
            ...
            case "p":
                ...
                guard !text.isEmpty else {
                    ...
                    // paragraph's clothes, as Calibre and InDesign
                    // exports write them — imports as the image it is.
                    if let image = element.firstDescendant(named: "img") {
                        visit(image, stretchID: stretchID)
                    }
                    return
                }
    ...
    /// the markers: CommonMark refuses emphasis that opens or closes on
    /// a space, and the export wraps punctuation and spaces in `<strong>`
    /// runs of their own — `<strong>Term</strong><strong>. </strong>`.
    /// Whitespace-only content passes through unmarked, and a run that
    /// meets the previous one back-to-back fuses with it.
    private static func appendInline(_ content: String, marker: String,
                                     to out: inout String) {
        ...
        var leading = String(content.prefix { $0.isWhitespace })
        let trailing = String(content.reversed().prefix { $0.isWhitespace }.reversed())
        if let last = out.last, last.isWhitespace {
            // The hoisted space would double one already there.
            leading = ""
        }
        // `*` is left out of fusing: a `**` already closing bold there
        // would be mistaken for an em marker.
        if leading.isEmpty, marker.count > 1, out.hasSuffix(marker) {
            ...
            case .element(let inner):
                ...
                case "strong", "b":
                    if inner.attributes["class"] == "speaker" {
                        ...
                        appendInline(content, marker: "**", to: &out)
                    }
                case "em", "i":
                    appendInline(content, marker: "*", to: &out)
                case "mark":
                    // Marked text (Author's orange Mark) travels as the
                    // ==…== convention; the readers paint it back.
                    appendInline(content, marker: "==", to: &out)
                case "code":
                    out += "`\(content)`"
                case "dfn":
                ...
                case "a":
                    if (inner.attributes["class"] ?? "").contains("ot-stretchtext") {
                        // The stretchtext toggle glyph (›) is chrome, not
...
private nonisolated final class XMLTree: NSObject, XMLParserDelegate {
    ...
    static func parse(_ data: Data) throws -> Element {
        let tree = XMLTree()
        let parser = XMLParser(data: sanitized(data))
    ...
    /// XML 1.0 forbids most control characters, but word-processed
    /// text carries them into exports — form feeds where page breaks
    /// once were, vertical tabs. They are whitespace at heart; parse
    /// with them folded to spaces rather than refusing the document.
    private static func sanitized(_ data: Data) -> Data {
        func isValidXML(_ scalar: Unicode.Scalar) -> Bool {
            switch scalar.value {
        ...
        guard text.unicodeScalars.contains(where: { !isValidXML($0) }) else { return data }
        var scalars = String.UnicodeScalarView()
        for scalar in text.unicodeScalars {
            scalars.append(isValidXML(scalar) ? scalar : " ")
        }
        return Data(String(scalars).utf8)
    }
