...
// Ported verbatim from Augmented Library's OrigamiReading.swift — the
// Origami reading mode's UI-free underpinnings: the view styles a
// document can be read in, the context-menu actions a reader can
// invoke, the section structure derived from a body's headings, and
// the citation a paragraph travels as when copied. All value types,
// every piece testable without a window. Two helpers are inlined
// (normalize from CitationMatcher, stableHash from PDFIdentity);
// otherwise unchanged — a fix here should be carried back.
...
nonisolated enum OrigamiContextAction: String, CaseIterable, Identifiable, Sendable {
    ...
    case copyViewSpec
    /// A W3C highlight on the paragraph, kept in the Augmented Library
    ...
    /// An empty string is an explicit "none"; decode falls back to the
    /// defaults only for nil (never stored).
...
nonisolated struct OrigamiViewState: Equatable, Sendable {
    ...
    func readableLine(in doc: LiquidDoc) -> String? {
        ...
        func title(_ headingID: String) -> String {
            doc.body?.first { $0.id == headingID }.map { "“\($0.text)”" } ?? headingID
        }
        var clauses: [String] = []
...
nonisolated enum OrigamiReading {
    ...
    static func citation(for paragraph: LiquidDoc.Paragraph,
        ...
        // the heading credited one.
        let sentence: String
        let address: String
        if let excerpt = doc.excerptOf {
            var line = ""
            if !doc.author.isEmpty,
               doc.author.caseInsensitiveCompare(excerpt.author) != .orderedSame {
                line += doc.author + ", in "
            }
            line += "\u{201C}\(excerpt.title.isEmpty ? "Untitled" : excerpt.title)\u{201D}"
            let credit = [excerpt.author, excerpt.date ?? ""]
                .filter { !$0.isEmpty }
                .joined(separator: ", ")
            if !credit.isEmpty { line += " (\(credit))" }
            sentence = line
            address = excerpt.id + "#" + paragraph.id
        } else {
    ...
    /// concept matcher's common ground (Augmented Library's
    /// CitationMatcher.normalize, inlined).
    ...
    /// derived documents (Augmented Library's PDFIdentity.hash, inlined).
    ...
    static func noteTokensResolved(_ text: String) -> String {
        ...
        for match in matches.reversed() {
            ...
            out = out.replacingCharacters(in: match.range,
                                          with: "[†](\(noteScheme):\(escaped))") as NSString
        }
