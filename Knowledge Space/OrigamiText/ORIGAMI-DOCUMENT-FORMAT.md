...
- `body` — ordered array of paragraphs: `id` (string, unique within the document), `text` (string), optional `heading` (integer 1–3; clamp out-of-range values), optional `speaker` (string — see below). Three further optional paragraph fields carry the Origami EPUB profile's structures (shared with Origami Text and Augmented Library): `tableID` names the entry in the document's `tables` pool this paragraph stands for — its `text` keeps a pipe-table rendering as the plain-text fallback; `stretchID` marks the paragraph as stretchtext (expandable detail — consecutive paragraphs sharing an id open and close together; a reader without stretch support shows the words plainly, which loses nothing); `contributingAuthors` credits a section's own writer when a heading carries one.
...
- `references` — the document's **external citation records**: an array of `{ "id": "<stable identifier>", "bibtex": "@book{…}" }`, each a verbatim BibTeX entry. Citations of *library* documents ride on `links` (which know their address and carry `bibtex` there); `references` holds the works outside the library — books, papers, web pages. Concepts' `citationIdentifiers` and spatial layouts may reference these ids, and publication emits the records into the Visual-Meta `@{references}` block (§7) alongside the links'. A record may additionally carry two optional fields, both read from the source and never invented by a reader: `citedAs` — the citation's display text exactly as its author wrote it (an imported EPUB's in-text anchor content, e.g. `"(Atzenbeck, 2023)"`), which readers show verbatim in the author–date style — and `number`, the record's 1-based place in the source's own reference list (an EPUB's `data-citation-number`). Numbered citation styles display `number` and never renumber client-side, so the in-text citations and the source's visible list always agree.
...
- `action` — the document's action standing, the lifecycle axis orthogonal to filing: absent means nothing is asked of it; the recommended tokens are `todo`, `in progress`, `done`, and `cancelled`. An open vocabulary like `documentType`: unknown tokens are preserved verbatim. Travelling in the file, every device agrees on a note's standing.
- `connections` — drawn connections between nodes of the concept/citation pool, as lines on a source Map: an array of `{ "from": "<node id>", "to": "<node id>" }`. Without them a layout is dots without lines. A connection naming an unknown node id is tolerated, like a layout position.
...
- `tables` — the document's **live tables**, referenced by paragraphs' `tableID`: an array of `{ "identifier": "…", "rowCount": 2, "columnCount": 3, "cells": [[{ "value": "…", "formula": "…" }]] }`, cells row-major, `formula` optional (a spreadsheet formula behind a computed `value`). The referencing paragraph's `text` carries a pipe-table fallback, so a reader without table support loses nothing. Matches the Origami EPUB profile's `tables` entry, so the two forms round-trip.
- `assets` — **binary assets (images) carried in the document itself**: an array of `{ "id": "img1", "filename": "figure.png", "mediaType": "image/png", "dataBase64": "…", "alt": "…" }`. The body refers to an asset with a lone image-marker paragraph `![alt](asset:<id>)`; the id is stable so the marker and an exported EPUB's image file line up. A marker whose bytes are absent keeps its reference visible rather than vanishing.
- `excerptOf` — set when this document is a **section carved out of another**: `{ "id": "<original address>", "title": "…", "author": "…", "date": "…", "headingID": "<the extracted heading's paragraph id>", "headingText": "…" }`. The excerpt keeps the original's paragraph ids, so citations made from it address the original, and a reader holding the original can open it at the right place.
...
Each link: `to` (an address), optional `fragment` (paragraph id in the target), optional `rel`, optional `bibtex`, optional `span`, optional `locator`.
...
- `locator` — where in the target *work* the link points, as a free-form human string: `"p. 37"`, `"chapter 3"`, `"12:40"`. The rung below `fragment` for targets whose insides have no paragraph ids — wrapped files (§8) above all. Prose, not structure, per principle 2; readers display it beside the link and never parse it.
...
| `annotates` | comments on a place in the target (usually with a `locator`) | gathered under the target as its annotations |
...
**EPUBs are read whole, not wrapped.** An imported EPUB (the Origami EPUB profile of Origami Text, Author, and Augmented Library — or a plain EPUB, chapter by chapter) becomes a **full body document**: its text, glossary (`concepts`), citation pool (`links` and `references`), Map views (`layouts`, `connections`), `tables`, images (`assets`), and `excerptOf` provenance all aboard, under the EPUB's own carried address where it has one so citations to the work resolve wherever it arrives. The EPUB itself is copied into the reader's **EPUB Library** folder (their own folder of EPUBs, granted once — the Reader Library's twin) under the readable `<slug>--<id>.epub` name that pairs it with its record; with no EPUB Library chosen it is kept beside the document as `<id>.epub`. Re-importing a newer export refreshes the document in place; the reader's filing, action standing, and shelving stay.
...
## Reader annotations (sidecar, not part of the document)

A reader's highlights and comments are **never written into the document** — the document is the author's; the annotations are the reader's. They live in an optional sidecar beside the document file, named `<id>.annotations.jsonld`, holding a W3C Web Annotation Collection (https://www.w3.org/TR/annotation-model/ — the same model Hypothesis uses). Each annotation targets the document by its address (`origamitext://open/<id>`) and anchors through a selector ladder: a `FragmentSelector` carrying the stable paragraph id first, a `TextQuoteSelector` (exact words with prefix/suffix context) as the re-anchoring fallback, a `TextPositionSelector` as a hint. Applications that do not understand the sidecar simply ignore it; deleting it deletes only the reader's notes. Introduced by Augmented Library, adopted by Knowledge Space's book reader (2026-08-19).

A reader's what-if numbers over a live table travel the same way: one annotation per changed cell, motivation `editing`, anchored to the table's paragraph id, its body the readable line `B3 = 7.5` (the cell's spreadsheet address, then the reader's number). The table's own values and formulas stay the document's; a reader without table support sees an ordinary, readable annotation.
