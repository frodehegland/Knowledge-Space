...
// Ported verbatim from Augmented Library's ViewSpecification.swift —
// keep synced; a fix here should be carried back.
...
// inside it for contexts that want one line. The format is documented
// in VIEW-SPECIFICATION.md at the Augmented Library repository root.

/// Any JSON value — the view variables of different kinds of views
/// need strings, numbers, lists, and nested shapes alike.
public nonisolated indirect enum JSONValue: Hashable, Sendable {
    case string(String)
    case number(Double)
    case bool(Bool)
...
public nonisolated struct ViewSpecification: Equatable, Sendable {
    ...
    /// "globe" for Interatlas's Earth; others as apps define them.
    ...
    public static func parse(_ text: String) -> ViewSpecification? {
        ...
        let view: [String: JSONValue] = if case .object(let values)? = raw["view"] { values } else { [:] }
    ...
    private func queryValue(_ value: JSONValue?) -> String? {
        switch value {
...
extension OrigamiReading {
    ...
    public static func viewSpecification(for paragraph: LiquidDoc.Paragraph,
        ...
        var view: [String: JSONValue] = ["style": .string(state.style.rawValue)]
        if !state.closedSections.isEmpty {
            view["closed"] = .array(state.closedSections.map(JSONValue.string))
        }
        if !state.openStretch.isEmpty {
            view["open"] = .array(state.openStretch.map(JSONValue.string))
        }
        if let focus = state.focusSectionID {
            view["focus"] = .string(focus)
        }
        // An excerpt's view locates in the original document — the
        // paragraph ids are the original's.
        let documentID = doc.excerptOf?.id ?? doc.id
        return ViewSpecification(kind: "origami-reading",
                                 generator: generator,
                                 location: documentID + "#" + paragraph.id,
                                 view: view)
    }
}
