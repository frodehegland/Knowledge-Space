...
/// Colors shade from one end to the other; width carries weight.
...
/// Any collection of identifiable items laid out as cards in a volume.
/// Cards seed flat — z starts at zero; depth is the reader's to give —
/// and the same pinch that moves a card on the plane pulls it toward you
/// or pushes it away in Z. Each card reads from the front and the back.
/// The arrangement persists per layout name, and every placement is
/// kept: the space is where you left it, next session included.
struct SpatialCardPlane<Item: Identifiable, CardFace: View, Extras: View>: View where Item.ID == String {
    ...
    /// First arrangement, essentially 2D: a centered grid, or a circle
    /// starting at the top. Only unplaced items are seeded.
...
/// The Knowledge Space: the library laid out in a volume, where the hand
/// can pull a card toward you or push it away in Z. Every document is a
/// small card, title then author, readable from the front and the back.
/// Links draw as threads through the volume, staying attached to their
/// cards at any depth. Double-tap a card to open it in the library
/// window; pinch-drag to move it in all three dimensions.
struct KnowledgeSpaceView: View {
    ...
    /// an edit elsewhere does not break a thread. A selected card's
    ...
    private var connections: [SpatialCardConnection] {
        ...
        for doc in docs {
            for link in doc.links {
                let target = state.index.latestRevision(of: LiquidAddress.canonical(link.to))
    ...
    var body: some View {
        SpatialCardPlane(layoutName: "KnowledgeSpaceLayout",
                         ...
                         countNoun: "documents",
                         connections: connections,
                         resolveID: { state.index.latestRevision(of: $0) },
                         onOpen: { state.open(id: $0.id) },
                         onSelect: { doc in
        ...
        .overlay {
            if docs.isEmpty {
                ContentUnavailableView("Nothing in the Space",
                                       ...
                                       description: Text("Choose a library folder in the library window."))
            }
        }
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    Text("Cards show the document's title, then its author, on both faces. Double-tap any card to open it in the library window.")
                }
            }
            .toolbar {
